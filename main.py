...
class AirDrawApp:
    ...
    def create_canvas_if_needed(self, frame) -> None:
        if self.canvas is None or self.canvas.shape != frame.shape:
            self.canvas = np.zeros_like(frame)

    def draw_toolbar(self, frame) -> None:
    ...
    def handle_toolbar_selection(self, point: Point) -> bool:
        x, y = point
        if y > HEADER_HEIGHT:
            return False
        for name, (x1, y1, x2, y2) in TOOLBAR:
    ...
    def save_canvas(self) -> Optional[str]:
        ensure_output_dir()
        ts = int(time.time())
